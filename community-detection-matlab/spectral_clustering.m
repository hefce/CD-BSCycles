...
% CONCEPT: Introduced the normalization process of S matrix(D-1/2 A D-1/2), 
% eigenvectors orthonormal conversion and clustering by kmeans 
function [IDXx,Cc]=spectral_clustering(S)
...
% compute the normalized laplacian / S matrix (method 1)
%NL1 = D^(-1/2) .* L .* D^(-1/2);
for i=1:size(S,1)
    for j=1:size(S,2)
...
% NL2 = eye(size(S,1),size(S,2)) - (D^(-1/2) .* S .* D^(-1/2));
