function [assignment_multicut,param_string]=cluster_linkage(S,k,link_method)
...
% assignment_multicut( 1,n ) vector of integers 1:k assigning the points to clusters
  ...
  for i=1:(n-1)
	...
	index=index+num_j;
% 	for j=(i+1):n
% 	  sim_vector(index)=1/(S(i,j)+1e-10); % linkage expects distances 
% 	  index=index+1; 
% 	end
  end
  ...
  % do the clustering 
  assignment_multicut=cluster(htree,k); 
  if nargout >1 
	param_string='';
  end
